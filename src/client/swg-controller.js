...
module.exports = class SwgController {
	...
	defaultOnwardEntitledJourney ({ promptLogin=false, consentRequired=false }={}) {
		if (promptLogin) {
			...
			this.overlay.show(`<p>It looks like you already have an FT.com subscription with Google.</p><p><a class="overlay__button" href="${loginHref}">Please login</a></p><p><small>code: ENTITLED_LOGIN_REQUIRED</small></p>`, 'google-ft');
		} else {
			...
			this.overlay.show(`<p>It looks like you already have an FT.com subscription with Google.</p><p>You have been logged in.</p><p><a class="overlay__button" href="${consentHref || contentHref}">Go to content</a></p><small>code: ENTITLED_LOGIN_SUCCESS</small></p>`, 'google-ft');
		}
	}
	...
	onwardSubscriptionErrorJourney () {
		this.overlay.show('<p>Something went wrong!</p>', 'ft');
	};
